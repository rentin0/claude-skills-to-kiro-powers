...
class SkillsDownloader {
  ...
  async downloadSkill(skillName) {
    ...
    try {
      ...
      throw new Error(`\u30B9\u30AD\u30EB\u306E\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9\u306B\u5931\u6557\u3057\u307E\u3057\u305F: ${error instanceof Error ? error.message : error}`);
    }
  }
...
var options = {
  ...
  convert: { type: "string", short: "c" },
  output: { type: "string", short: "o" }
};
var parsed = parseArgs({ args, options, allowPositionals: true });
async function main() {
  ...
  try {
    if (parsed.values.install) {
      ...
      const outputDir = parsed.values.output || `~/.kiro/powers/${skillName}`;
      ...
      const outputDir = parsed.values.output || `~/.kiro/powers/${skillPath.split("/").pop()}`;
